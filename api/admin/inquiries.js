...
async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    const resp = await fetch(
      `${SUPABASE_URL}/rest/v1/inquiries?select=id,created_at,name,email,phone,event_date,event_type,venue_city,status,selected_package,estimated_total,completed,last_step,sourcePage:raw_payload->>sourcePage,confirmationSent:raw_payload->>confirmation_sent${filter}&order=created_at.desc&limit=${limit}`,
      { headers: baseHeaders }
    );
    if (!resp.ok) {
